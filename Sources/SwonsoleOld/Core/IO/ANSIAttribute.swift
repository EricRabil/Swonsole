public enum ANSIAttr: UInt8, CaseIterable, CustomStringConvertible {
    /* text styling */
    case normal         = 0
    case bold           = 1
    case dim            = 2
    ...
    public static let strings: [ANSIAttr: String] = allCases.buildDict {
    ...
    @inlinable static func count<Text: StringProtocol>(textWithoutAttributes text: Text, ansiCount: inout Int) -> Int {
        var count = 0
    ...
    @inlinable public var description: String {
        Self.strings[self].unsafelyUnwrapped
    }
}
...
public extension ANSIAttr {
    @inlinable var isStyle: Bool {
        switch self {
    ...
    static let openColorTerminator = CSI+"\(ANSIAttr.`default`.rawValue);\(ANSIAttr.onDefault.rawValue)m"
    
    func style<Text: StringProtocol>(_ text: Text) -> String {
        guard !text.isEmpty else {
            return ""
        }
        
        switch self {
        case .normal: return description + text
        case .`default`: return description + text
        case .onDefault: return description + text
        ...
        case .strike: return description + text + ANSIAttr.normal.description
        default: return description + text + Self.openColorTerminator
        }
    }
}
...
extension ANSIAttr {
    ...
    @usableFromInline static func strip<Text: StringProtocol>(_ text: Text) -> String {
        guard !text.isEmpty else { return String(text) }
